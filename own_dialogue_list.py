...
def upload_and_evaluate_tab():
    ...
    for idx in range(start, end):
        ...
        prev_q = int(prev.get("question", 5)) if str(prev.get("question", "")).isdigit() else 5
        prev_r = int(prev.get("realism", 5)) if str(prev.get("realism", "")).isdigit() else 5
        ...
        with st.form(f"own_eval_form_{idx}"):
            ...
            question_score = st.number_input(
                "질문의 적절성 (1~10)", min_value=1, max_value=10, value=prev_q, key=f"own_question_{idx}"
            )
            realism_score = st.number_input(
                "전체 대화의 현실성 (1~10)", min_value=1, max_value=10, value=prev_r, key=f"own_realism_{idx}"
            )
            evaluator = st.text_input(
                "평가자 이름 또는 ID", value=prev_eval, placeholder="예: hong_gildong", key=f"own_evaluator_{idx}"
            )
            ...
            if submitted:
                if not evaluator.strip():
                    ...
                    update_own_evaluation(idx, ktas_score, question_score, realism_score, evaluator)
